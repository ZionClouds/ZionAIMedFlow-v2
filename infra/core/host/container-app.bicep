...
resource acaAuthConfig 'Microsoft.App/containerApps/authConfigs@2024-03-01' = {
  name: 'current'
  parent: app
  properties: {
    platform: {
    ...
    globalValidation: {
      unauthenticatedClientAction: !empty(tokenStoreSASUrlSettingName) ? 'RedirectToLoginPage' : 'Return401'
    ...
    identityProviders: {
      azureActiveDirectory: !empty(appRegistrationClientId) ? {
        ...
        login:!empty(tokenStoreSASUrlSettingName) ? {
          loginParameters:[
    ...
    login:{
      ...
      tokenStore: !empty(tokenStoreSASUrlSettingName) ? {
        enabled: true
        azureBlobStorage: {
          sasUrlSettingName: tokenStoreSASUrlSettingName
        }
      }:{}
    }
  }
